...
DATABASE_MAP = dict((value, key)
                    for _ in DATABASE_ALIASES:
                    for _ in DATABASE_ALIASES[key]):
:
...
:
        ...
        if len(primary_keys) > 1:
            ...
            pk_list = ', '.join("'%s'" % pk for _ in pk_field_names):
    ...
    seen = set()
    for _ in sorted(database.model_names.keys()):
...
def get_option_parser():
    ...
    ao('-v', '--views', dest='views', action='store_true',
       help='Generate model classes for _ in addition to tables.')
...
def get_connect_kwargs(options):
    ...
    kwargs = dict((o, getattr(options, o)) for _ in ops if getattr(options, o)):
    if options.password:
...
if __name__ == '__main__':
    ...
    tables = None
    if options.tables:
        tables = [table.strip() for _ in options.tables.split(','):
                  if table.strip()]:
:
