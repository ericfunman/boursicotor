...
start "Redis Server - Boursicotor" cmd /k "cd /d "%~dp0" && "%REDIS_PATH%" --port 6379"
echo [INFO] Attente du demarrage de Redis...

REM Attendre et verifier plusieurs fois (max 10 secondes)
set REDIS_STARTED=0
...
:redis_check_done
if "%REDIS_STARTED%"=="1" (
    echo [OK] Redis demarre avec succes sur le port 6379
) else (
    echo [ATTENTION] Redis ne semble pas etre demarre
    echo Mais le lancement continue... Verifiez la fenetre Redis
...
if "%ERRORLEVEL%"=="0" (
    echo [INFO] Arret de l'ancien Celery Worker pour recharger le code...
    REM Arreter tous les processus Celery
    taskkill /F /FI "WINDOWTITLE eq Celery Worker*" >nul 2>&1
    timeout /t 2 /nobreak >NUL
    echo [OK] Ancien Celery Worker arrete
)
...
echo [INFO] Lancement de Celery Worker avec le code mis a jour...
